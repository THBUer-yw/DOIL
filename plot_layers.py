...
select_method_names = ['layers_2','layers_3','layers_4','layers_5']
...
def get_filename_dict(base_dir):
    ...
    for dir in os.listdir(base_dir):
        ...
        if os.path.isdir(os.path.join(base_dir, dir)):
            element = dir.split("_")
            num_layers = element[42]
            method = "layers_"+num_layers
            env = element[0]
            seed = int(element[6])
...
def plot(logdir):
    ...
    for i in range(len(filenames)):
        ...
        os.makedirs(r"./plot_layers", exist_ok=True)
        ...
        g = sns.lineplot(x=df.step, y="reward", data=df, hue="method", style="method", dashes=False, palette=palette,
            hue_order=['layers_2','layers_3','layers_4','layers_5'])
        plt.tight_layout()
        if env != "Ant-v2":
            ax.legend_.remove()
        fig.savefig("./plot_layers/{}.png".format(env), bbox_inches='tight')



if __name__ == '__main__':
    print(os.getcwd())
    log_dir = r"./plot_layers"
    plot(log_dir)
